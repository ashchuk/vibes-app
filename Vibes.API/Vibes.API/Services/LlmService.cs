...
public class LlmService : ILlmService
{
    ...
    public async Task<ExtractedEvent> ExtractFirstEventFromPlanAsync(string planText, string userTimeZoneId)
    {
        ...
        var prompt = $"""
                      Ты — AI-парсер. Твоя задача — проанализировать текстовое описание планов от пользователя и извлечь информацию о ПЕРВОМ, НАИБОЛЕЕ ВАЖНОМ И КОНКРЕТНОМ событии, которое пользователь хочет ДОБАВИТЬ.
                      Это событие описано в первой части сообщения.
                      **Игнорируй события, которые уже существуют в календаре.** Твоя цель — найти НОВОЕ событие из текста пользователя.
                      Верни результат в формате JSON.

                      - **title**: Точное название события, которое хочет добавить пользователь.
                      - **startDateTime**: Полная дата и время начала в формате YYYY-MM-DD HH:mm.
                      ...
                      **Правила определения даты и времени:**
                      - Если пользователь говорит "завтра", используй завтрашнюю дату.
                      - Если пользователь говорит "в понедельник", "во вторник" и т.д., используй дату следующего соответствующего дня недели.
                      - Если пользователь указывает только время (например, "в 9 утра"), используй сегодняшнюю дату, если это время еще не прошло. Если прошло — используй завтрашнюю.
                      - Если в тексте нет явных событий, которые можно добавить, верни JSON с "title": "null".
